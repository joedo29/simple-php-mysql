...
load data local infile'/var/www/html/apiapps/data/user.txt' INTO table users;
...
load data local infile '/var/www/html/apiapps/data/book.txt' INTO table books;
...
load data local infile '/var/www/html/apiapps/data/item.txt' INTO table item;-- Insert data into 'item' table, no books are sold yet
...
in varAuthor varchar(225),in varPublisher varchar(225),in varYear_published int, in varBook_subject varchar(100))
...
-- when i added this book the second time "Linear Algebra and Its Applications (5th Edition)", it returned the similar item that I already have in the system. Also, duplicate item can't be added to the system. Is it intended?
...
load data local infile '/var/www/html/apiapps/data/transaction.txt' into table transaction_table;
...
-- create report 3, returns all the books that are available for sale, havnt been sold yet, 3 books are available for sale, rest are sold after running to queries above
